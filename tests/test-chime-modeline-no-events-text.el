...
(ert-deftest test-chime-modeline-no-events-text-boundary-no-events-at-all-returns-nil ()
  "Test that no events at all returns nil regardless of setting.

When there are zero events (not just beyond lookahead, but none at all),
the modeline should be nil regardless of chime-modeline-no-events-text."
  (test-chime-modeline-no-events-text-setup)
  (unwind-protect
      (let* ((now (test-time-now))
